...
while ($rows = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  echo '<td><img src="../upload/' .
    htmlentities($rows['profile_image']) . '" width="50px" /></td>';
  // the href should link to update.php
  // add a querystring sid in the url referring to the column portfolio_id
  echo '<form id="frmdel" action="deleteuser.php?sid=' . $rows["user_id"] .'" method="post">
          <td>
            <button type="submit" name="btndel" onclick="return confirmBlockUser()" 
              class="col-12 btn btn-outline-warning btn-lg mx-auto">
              Block user
            </button>
          </td>
        </form>';
