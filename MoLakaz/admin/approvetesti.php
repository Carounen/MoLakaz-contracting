...
<tbody>
<?php
$stmt = $pdo->query("SELECT u.firstname,t.testimonial_id, t.testimonial, t.rating, t.status
FROM user u
JOIN testimonial t ON u.user_id = t.user_id WHERE t.status = 0");
while ($rows = $stmt->fetch(PDO::FETCH_ASSOC)) {
    echo ("<tr>");
echo "<td>" . htmlentities($rows['rating']) . "</td>";


echo "<td>" . htmlentities($rows['firstname']) . "</td>";


echo "<td>" . htmlentities($rows['testimonial']) . "</td>";

//the href should link to update.php
//add a querystring sid in the url referring to the column portfolio_id
echo '<form id="frmdel"
action="approve.php?sid=' . $rows["testimonial_id"] .'" method="post">
