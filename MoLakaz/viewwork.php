...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM portfolio WHERE portfolio_id = :portId");
    $stmt->bindValue(':portId', $portId, PDO::PARAM_INT);
    $stmt->execute();
    ...
    <div class="row mt-3">
        <div class="col-md-4">
            ...
            <div class="rounded-circle overflow-hidden border border-primary img-container">
                <img src="upload/<?php echo htmlentities($portfolio["gallery"]); ?>" class="img-fluid profile-img" alt="Profile Image">
            </div>        </div>
        <div class="col-md-8">
            <h3>Work Profile</h3>
            <form>
                <div class="form-group">
                    <label for="firstname">Type of work</label>
                    <input type="text" class="form-control" id="firstname" value="<?php echo htmlentities($portfolio["description"]); ?>" readonly>
                </div>
                <div class="form-group">
                    <label for="lastname">Date Done</label>
                    <input type="text" class="form-control" id="lastname" value="<?php echo htmlentities($portfolio["date"]); ?>" readonly>
                </div>
