...
$stmt = $pdo->prepare("SELECT * from contractor where contractor_id=:coid");
$stmt->execute(array(":coid" => $_SESSION["coid"]));
$srow = $stmt->fetch(PDO::FETCH_ASSOC);

$pic = htmlentities($srow['profile_img']);
...
if (isset($_POST['btnupdate'])) {
    ...
    //USERS
    $stmt2 = $pdo->prepare("SELECT * FROM contractor
    where email = :em and contractor_id != :coid");
    $stmt2->execute(
    array(
    ":em" => $_POST['email'],
    ':coid' => $_SESSION['coid']
    ...
    $sql = "UPDATE contractor set firstname =:fn, profile_img=:filen, lastname=:ln, 
 email=:email where contractor_id=:coid";
    ...
    move_uploaded_file($_FILES["profile_img"]["tmp_name"], "../upload/" .
    $filename);
